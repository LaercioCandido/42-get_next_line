...
/*   Updated: 2020/05/28 22:16:14 by lcandido         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*cleanline(char **line, char *s_line, int index_newline)
{
	...
	if (index_newline >= 0)
	{
		...
		*line = (char*)ft_calloc(ft_strlen(s_line) + 1, sizeof(char));
		l_temp = (char*)ft_calloc(ft_strlen(&s_line[index_newline + 1]) + 1, sizeof(char));
		ft_strlcpy(*line, s_line, ft_strlen(s_line) + 1);
		ft_strlcpy(l_temp, &s_line[index_newline + 1], ft_strlen(&s_line[index_newline + 1]) + 1);
		free(s_line);
		s_line = NULL;
		return (l_temp);
	}
